# GymPybulletDeepUAVControl

## Abstract Thesis
This project presents the design and implementation of a position controller for a multi-rotor UAV based on deep neural networks and trained by supervised learning, taking as reference a PID controller. The process of selection of the simulation environment, the controller and the selected model is detailed. Likewise, control trajectory evaluations are carried out for the construction of a data set to train the model. Different neural network architectures are trained, using the \textit{Hyperband} algorithm to determine the best hyperparameters. Finally, the performance of the trained controller with respect to the base controller is evaluated by means of the time response with different control signals. As a final product we present: the data set of the reference controller, a repository with the programs made for the development and analysis, and the neural network model.
...
# gym-pybullet-drones (Original Repository)
